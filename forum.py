...
def post_comment(user_id, stock_ticker, timestamp, content, parent_id=None):
    ...
    # If no parent_id is provided, comment is a parent comment
    if not parent_id:
        insert_query = "INSERT INTO parentcomment (stock_ticker, author_id, time_stamp, content) VALUES (%s, %s, %s, %s)"
        values = (stock_ticker, user_id, timestamp, content)
    # Otherwise, using the provided parent id, it is a child comment
    else:
        insert_query = "INSERT INTO childcomment (parent_id, stock_ticker, author_id, time_stamp, content) VALUES (%s, %s, %s, %s, %s)"
...
if __name__ == "__main__":
    print(post_comment("0ee69cfc-83ce-11eb-8620-0a4e2d6dea13", "IBM", 1616810169114,
          "fuck tesla. Okay how are you doing?", "2f477d66-8e93-11eb-b5ed-0a4e2d6dea13"))
