...
def get_walkforward_prediction(model, initial_data, n_steps=60, max_intervals=120):
    ...
    y_data = featurise_single_series(
        initial_data[-n_steps:], n_entries=1, n_steps=n_steps, n_features=1
    )
...
def get_walkforward_cnn(model, initial_data, n_steps=60, n_seq=2, max_intervals=120):
    ...
    predictions = []
    x_data_i = featurise_single_series(
        initial_data[-n_steps:], n_entries=1, n_steps=n_steps, n_features=1
    )
    for i in range(max_intervals):
        x_data = featurise_cnn(
            np.copy(x_data_i), n_entries=1, n_steps=n_steps, n_seq=n_seq, n_features=1
        )
...
class Retrieve_Prediction(Resource):
    ...
    @PREDICTION_DAILY_NS.expect(
        prediction_daily_params_model(PREDICTION_DAILY_NS), validate=True
    )
    ...
    def post(self):
        ...
        try:
            ...
            if len(initial_data.shape) > 1:
                abort(
                    500,
                    f"Data received is {len(initial_data.shape[0])}-dimensional but data sequence must be 1-dimensional",
                )
            if not initial_data.dtype in [np.dtype("int"), np.dtype("float")]:
                abort(
                    500,
                    f"Data received is of type {initial_data.dtype} but data sequence must be of type int or float",
                )
            ...
            if inf_mode in inf_modes and data_length >= inf_modes[inf_mode]["length"]:
                ...
                initial_data = (
                    initial_data[-req_len:] if data_length > req_len else initial_data
                )
                predictions = []
                if inf_mode == "walk_forward":
                    predictions = get_walkforward_prediction(
                        models["lstm_vanilla"], initial_data
                    )
                elif inf_mode == "multistep_series":
                    predictions = get_multistep_vanilla(
                        models["lstm_vanilla"], inf_mode, initial_data
                    )
                elif inf_mode == "cnn":
                ...
                abort(
                    500,
                    f"Data length is {data_length} but required data length for mode \"{inf_modes[inf_mode]['name']}\" is {inf_modes[inf_mode]['length']}",
                )
            else:
                abort(500, f'Inference mode "{inf_mode}" not supported')
