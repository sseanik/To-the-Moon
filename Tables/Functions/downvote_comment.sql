CREATE OR REPLACE FUNCTION downvote_comment(id_user UUID, id_comment UUID) RETURNS TABLE(
        comment_id UUID,
...
SET downvote_user_ids = array_remove(forum_comment.downvote_user_ids, id_user)
WHERE forum_comment.comment_id = id_comment;
ELSE
UPDATE forum_comment
SET downvote_user_ids = array_append(forum_comment.downvote_user_ids, id_user),
    upvote_user_ids = array_remove(forum_comment.upvote_user_ids, id_user)
WHERE forum_comment.comment_id = id_comment;
END IF;
RETURN QUERY
SELECT c.comment_id,
