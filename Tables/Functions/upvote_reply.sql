CREATE OR REPLACE FUNCTION upvote_reply(id_user UUID, id_reply UUID) RETURNS TABLE(
        reply_id UUID,
...
UPDATE forum_reply
SET upvote_user_ids = array_remove(forum_reply.upvote_user_ids, id_user)
WHERE forum_reply.reply_id = id_reply;
ELSE
UPDATE forum_reply
SET upvote_user_ids = array_append(forum_reply.upvote_user_ids, id_user),
    downvote_user_ids = array_remove(forum_reply.downvote_user_ids, id_user)
WHERE forum_reply.reply_id = id_reply;
END IF;
RETURN QUERY
SELECT c.reply_id,
